...
import { computeAuditChainHash, getAuditChainHead } from './content-guard.js';
import logger from '../utils/logger.js';

export async function audit(userId: string | null, action: string, details?: Record<string, unknown>, platform?: string) {
  try {
    // ── Tamper-evident hash chain (Gemini recommendation) ──
    const chainHash = computeAuditChainHash(action, JSON.stringify(details ?? {}), Date.now());
    ...
    await logAction(safeUserId, action, undefined, {
      ...details,
      originalUserId: userId,
      _chainHash: chainHash,
      _chainHead: getAuditChainHead(),
    }, platform);
  } catch (err: unknown) {
